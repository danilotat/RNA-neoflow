rule align:
    ...
    output:
        temp(config['OUTPUT_FOLDER'] + config["datadirs"]["mapped_reads"] + "/" + "{patient}_Aligned.out.bam")
...
rule sortAlign:
    input:
        config['OUTPUT_FOLDER'] + config["datadirs"]["mapped_reads"] + "/" + "{patient}_Aligned.out.bam"
    output:
    ...
    resources:
        ...
        ncpus = 2
    log:
        config['OUTPUT_FOLDER'] + config["datadirs"]["logs"]["sort_bam"] + "/" + "{patient}.log"
    shell:
        '''
...
rule indexSortAligned:
    input:
        config['OUTPUT_FOLDER'] + config["datadirs"]["mapped_reads"] + "/" + "{patient}_Aligned.sortedByCoord.out.bam"
    output:
        temp(config['OUTPUT_FOLDER'] + config["datadirs"]["mapped_reads"] + "/" + "{patient}_Aligned.sortedByCoord.out.bam.bai")
    conda:
        "../envs/samtools.yml"
    params:
        prefix = config['OUTPUT_FOLDER'] + config["datadirs"]["mapped_reads"] + '/{patient}_'
