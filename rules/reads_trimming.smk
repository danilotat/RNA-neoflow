...
rule trimming_pe:
    input:
        sample=[config["resources"]["FASTQ"]+"/"+"{patient}_1.fastq.gz",config["resources"]["FASTQ"]+"/"+"{patient}_2.fastq.gz"]
    output:
        trimmed=[config["datadirs"]["trimmed_reads"]+"/"+"{patient}_1.fastq.gz",config["datadirs"]["trimmed_reads"]+"/"+"{patient}_2.fastq.gz"],
