    ...
    with open(project+'.json', 'r') as json_mdata:
        ...
        with open(project+'_metadata.csv', 'w') as oput_file:
            ...
            for x in accession_list:
                ...
                oput_file.write(','.join([x,sample_attr["gender"],sample_attr['phenotype'], str(sample_attr['subject_id']+'\n')]))
...
if __name__ == "__main__":
    ...
    if len(ftp_url_list) > 1:
        download_choice = input("I've found {} files. Would you like to download them? (Y/N)\t".format(len(ftp_url_list)))
